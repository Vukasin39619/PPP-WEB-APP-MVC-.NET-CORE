...
namespace PPP___ProjekatPokusaj2.Infrastructure.Implementations
{
    ...
    public class RacunRepository : IRacunRepository
    {
        ...
        public RacunRepository(MyAppDbContext context)
        {
            _context = context;
        }

        public async Task Add(RacunBO model)
        {
        ...
        public async Task<IEnumerable<RacunBO>> GetAll()
        {
            var ulice = await _context.Ulica.ToListAsync();
            return ulice;
        }

        public async Task<RacunBO> GetById(int id)
        {
            return await _context.Ulica.FindAsync(id);
        }

        public async Task Update(RacunBO model)
        {
